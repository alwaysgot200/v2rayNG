...
[![GitHub commit activity](https://img.shields.io/github/commit-activity/m/2dust/v2plus)](https://github.com/2dust/v2plus/commits/master)
[![CodeFactor](https://www.codefactor.io/repository/github/2dust/v2plus/badge)](https://www.codefactor.io/repository/github/2dust/v2plus)
[![GitHub Releases](https://img.shields.io/github/downloads/2dust/v2plus/latest/total?logo=github)](https://github.com/2dust/v2plus/releases)
...
v2plus 是一款面向 Android 的图形化客户端，集成了代理协议解析、配置管理、VPN/本地代理服务与测速统计功能。应用通过 gomobile 生成的 `libv2ray.aar` 集成 Xray/V2Ray 核心，提供易用的界面与路由控制能力。
...
- `v2plus/`：Android 应用主工程（Kotlin/Java）。
...
- Gradle：使用仓库自带 Gradle Wrapper（`v2plus/gradlew`）
...
```
cd /Volumes/exfat/code/v2plus/AndroidLibXrayLite
```
...
```
mkdir -p /Volumes/exfat/code/v2plus-a/v2plus/app/libs
cp -f /Volumes/exfat/code/v2plus-a/AndroidLibXrayLite/libv2ray.aar /Volumes/exfat/code/v2plus/v2plus/app/libs/
```

2. `v2plus/app/build.gradle.kts` 已包含：
...
```
cd /Volumes/exfat/code/v2plus/v2plus
```
...
```
v2plus/app/build/outputs/apk/<flavor>/debug/
```

示例文件名：

- `v2plus_1.10.26-fdroid_arm64-v8a.apk`
- `v2plus_1.10.26-fdroid_armeabi-v7a.apk`
- `v2plus_1.10.26-fdroid_x86.apk`
- `v2plus_1.10.26-fdroid_x86_64.apk`
- `v2plus_1.10.26-fdroid_universal.apk`
...
- `badvpn/`（tun2socks 等）：
  ...
  - 产物：对应架构的 `.so` 或可执行，按需放置于 `v2plus/app/src/main/jniLibs/<ABI>/` 或 `app/libs/`。
...
- 集成 AAR：复制到 `v2plus/app/libs/`，Gradle 会自动依赖引入。
- 构建 APK：在 `v2plus/` 目录执行 `./gradlew :app:assembleFdroidDebug` 或 `:app:assemblePlaystoreDebug`。
- 产物位置：`v2plus/app/build/outputs/apk/<flavor>/debug/`（按 ABI 输出或 `universal` 包）。
...
- 新增脚本：`scripts/build-optionals-macos.sh`，在 VS Code 终端中一键构建并自动复制到 `jniLibs/`。
  ...
  - 用法：
    - 构建 tun2socks + hev 并复制到 `v2plus/app/src/main/jniLibs/`：
...
- `libv2ray.aar` 缺失导致编译期 `Unresolved reference 'libv2ray'/'go.Seq'`：
  ...
  - 请确保按第 3–4 步生成并复制 AAR 到 `v2plus/app/libs/`。
...
- v2plus 可在 Android 模拟器运行；WSA 环境需授予 VPN 权限：

```
appops set [package name] ACTIVATE_VPN allow
```

更多使用与开发文档请参考我们的 [Wiki](https://github.com/2dust/v2plus/wiki)。
